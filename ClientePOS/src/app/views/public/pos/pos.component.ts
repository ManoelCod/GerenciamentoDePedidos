...
import { PosService } from 'src/app/core/services/api/pos.service'
...
export class PosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.firestore.getDocuments("category")
      ...
      .catch(err => {
        this.notify.error2("Ops, ocorreu um erro :(");
      });
  ...
  console.log('Dados enviados para a API:', JSON.stringify(data, null, 2));
    ...
    this.posService.create(data).subscribe(response => {
      Swal.fire("OK", "", "success").then(()=> location.reload());
      console.log('Dados salvos com sucesso:', response);
    }, error => {
      this.notify.error2("Ocorreu erro servidor :(");
    });
